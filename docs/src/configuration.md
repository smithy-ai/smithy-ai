...
| `COORDINATOR_FORGEJO_TOKEN` | — | API token for the coordinator actor. Without it a coordinator acts as smithy, and a feature's plan appears written by the agent implementing it |
...
| `COORDINATOR_GITHUB_TOKEN` | — | Personal access token for the coordinator actor |
...
| `COORDINATOR_GITLAB_TOKEN` | — | Access token for the coordinator actor |
...
coordinator, a task for smithy — so they need separate accounts. See
...
| `METRICS_PATH` | — | Append-only metrics log |
| `ADMIN_PASSWORD_HASH` | — | bcrypt hash for the dashboard's `admin` user. A random password is generated and printed at startup when unset |
...
| `JIRA_EMAIL` | — | Account email for API authentication |
| `JIRA_API_TOKEN` | — | API token |
| `JIRA_BOT_ACCOUNT_ID` | — | Account id assignment is detected against |
| `JIRA_WEBHOOK_SECRET` | — | Shared secret, sent as `X-Jira-Token` or `?token=` |
| `JIRA_REPO_FIELD` | — | Custom field holding `owner/repo[@base-branch]` |
| `JIRA_PLAN_APPROVED_LABEL` | `plan-approved` | Label that means approval |
| `JIRA_PLAN_APPROVED_STATUS` | — | Status transition that means approval |
