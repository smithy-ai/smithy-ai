...
def resolve_token(api: GitHubAPI, role: str) -> tuple[str, str]:
    """Validate a token and return (token, login)."""
    try:
...
def prompt_token(role: str, env: EnvFile, env_key: str) -> str:
    existing = env.get(env_key)
    if existing:
        print(f"    {env_key} already set in .env — using existing value")
        return existing
    token = getpass.getpass(f"GitHub token for {role} bot account ({env_key}): ").strip()
    if not token:
...
def main():
    ...
    smithy_token = prompt_token("smithy", env, "SMITHY_GITHUB_TOKEN")
    smithy_api = GitHubAPI(smithy_token, api_base)
    smithy_login = resolve_token(smithy_api, "smithy")

    # ── Step 2: Architect token ──────────────────────────────
    print()
    print("==> Validating architect bot token...")
    architect_token = prompt_token("architect", env, "ARCHITECT_GITHUB_TOKEN")
    architect_api = GitHubAPI(architect_token, api_base)
    architect_login = resolve_token(architect_api, "architect")
